...
export function markdown(options: MarkdownPluginOptions = {}): BunPlugin {
  ...
  return {
    ...
    setup(build) {
      ...
      // Custom renderer for blockquotes to support alternative callout syntax
      const _originalBlockquoteRenderer = renderer.blockquote
      renderer.blockquote = function (quote: any): string {
        ...
        if (calloutMatch) {
          ...
          // Map callout types to their corresponding alert classes
          ...
          // Get the icon for this alert type based on markedAlert's default icons
          const getIcon = (alertType: string): string => {
...
<p>${content.trim()}</p>
        ...
        // For regular blockquotes, use the original renderer
        return _originalBlockquoteRenderer.call(this, quote)
      }
